    ...
    async def load_all(self, settings) -> None:
        ...
        for cls in plugin_classes:
            ...
            if instance.config_key:
                plugin_config = getattr(settings.plugins, instance.config_key, None)
            ...
            self._tools.extend(unique_tools)
            self._plugin_tool_names[instance.name] = [t.schema["function"]["name"] for t in unique_tools]
        ...
        return [
            {
                ...
                "tools": self._plugin_tool_names.get(p.name, []),
            }
